...
<template>
    <div class="lesson_wrapper @container">
        <div class="grid gap-1 p-1 @[50vw]:grid-cols-[auto_1fr_auto] border-1 border-solid rounded-sm">
            ...
            <span class="flex gap-1 items-center  @[50vw]:order-1 @[50vw]:col-span-3">
                <UProgress class="@[50vw]:col-span-3"  v-model="lesson.progress" :max="lesson.duration" />
                <span class="hidden md:block">{{ totaltime }}</span>
            </span>
            
            <span class="text-right flex gap-2">
                <UIcon :name="lesson.bookmarked ? 'fa7-solid:bookmark' : 'fa7-regular:bookmark'" class="size-5" />
                <UIcon :name="lesson.progress >= lesson.duration ? 'fa7-solid:check-circle' : 'fa7-regular:check-circle'" class="size-5" />
            </span>
        </div>
    </div>
</template>
